...
void readStoneData() {
  ...
  switch(rd.id){
      ...
      case 0x1001: { //Button
          std::cout << "GData : " << intToHexa(abs(rd.id)) << " " << rd.command << " " << rd.name << " " << rd.type << "\n";
          std::string buttondemarrer = rd.name;
          std::cout << "Button : " <<  buttondemarrer.c_str() << "\n";
          if(strcmp(buttondemarrer.c_str(),"buttondemarrer")==0)
          buttonStart = 3;
          //std::this_thread::sleep_for(std::chrono::milliseconds(10));
...
void loop() {
  char strTemperature[64]; //recuperation de la temperature
  char strSeconde[64]; //recuperation du minuteur
  readStoneData();
  ...
  //           if(myTemp) temp = myTemp->readTemperature();
  //           Serial.println(temp);
  //           }
  int buttonActionT5 = myButtonT5->checkMyButton();
      ...
      //temperature du four afficher la temperature du senseur
  float temperature = myTemp->readTemperature();

    sprintf(strTemperature, "%g Celcius", temperature);
    delay(250);
    myStone->setLabel("labeltemperaturefour", strTemperature);
    delay(250);
  Serial.println(temperature);

      //si le button est appuyer, lance le minuteur
    if(buttonStart == 3)
    {
      buttonStart = 0;
      etatSysteme = 1;
    ...
    if(etatSysteme == 1){
        ...
        sprintf(strSeconde, "%d s", nbreSecondes);
    delay(1000);
    myStone->setLabel("labelsecondefour", strSeconde);
    Serial.println(nbreSecondes);
    if(nbreSecondes == 20)
    {
      etatSysteme = 2;
    }
